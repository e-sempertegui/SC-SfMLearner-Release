...
def train(args, train_loader, disp_net, pose_net, optimizer, epoch_size, logger, train_writer, epoch):
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(train_loader):
        ...
        if i == 0 or i % 1000 == 0:
            ...
            plot_path = '/cluster/scratch/semilk/NYU/results/photo_only_minor_fix_pt3/rgb_step_' + str(i) + '_epoch_' + str(epoch) + '.png'
            plt.axis('off')
            plt.savefig(plot_path, bbox_inches="tight", pad_inches=0, dpi=1200)

            # Save corresponding depth image
            plt.imshow(tgt_depth[0][0].detach().cpu().squeeze(), cmap=plt.cm.get_cmap("magma").reversed())
            plot_path = '/cluster/scratch/semilk/NYU/results/photo_only_minor_fix_pt3/depth_map_step_' + str(i) + '_epoch_' + str(epoch) + '.png'
            plt.axis('off')
            plt.savefig(plot_path, bbox_inches="tight", pad_inches=0, dpi=1200)
    ...
    plot_path = '/cluster/scratch/semilk/NYU/results/photo_only_minor_fix_pt3/depth_map_final_step_epoch_' + str(epoch) + '.png'
    plt.axis('off')
    plt.savefig(plot_path, bbox_inches="tight", pad_inches=0, dpi=1200)

    plt.imshow(torch.exp(uncertainty_tgt_final_step[0]).detach().cpu().squeeze(), cmap="hot")
    plot_path = '/cluster/scratch/semilk/NYU/results/photo_only_minor_fix_pt3/uncertainty_map_final_step_epoch_' + str(epoch) + '.png'
    plt.axis('off')
    plt.savefig(plot_path, bbox_inches="tight", pad_inches=0, dpi=1200)

    plt.imshow((tgt_img_final_step[0]*0.225 + 0.45).permute(1,2,0).detach().cpu().squeeze())
    plot_path = '/cluster/scratch/semilk/NYU/results/photo_only_minor_fix_pt3/rgb_final_step_epoch_' + str(epoch) + '.png'
    plt.axis('off')
    plt.savefig(plot_path, bbox_inches="tight", pad_inches=0, dpi=1200)
